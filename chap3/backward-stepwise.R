...
backward_stepwise <- function(X, y){
  ...
  return(list(removed_set=removed_set, save_RSS=save_RSS))
}

bsw_plot <- function(removed_set, save_RSS){
  plot(0:n, save_RSS, 
...
ret = backward_stepwise(X, y)
bsw_plot(ret$removed_set, ret$save_RSS)
