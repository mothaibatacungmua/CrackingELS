...
count_record = 0
num_record = 30
recorder = matrix(rep(0, 9), ncol = 9)
current_rss = y
beta = rep(0, 8)
epsilon = 0.001
...
for(i in 1:max_iter){
  ...
  if(count_record == (max_iter/num_record) || prev_beta[argmax] == 0.0){
    recorder = rbind(recorder, c(beta, l1_norm/i))
...
colnames(recorder) = c(ln[1:8], "L1-norm")

min_beta = min(recorder[,1:8])
max_beta = max(recorder[,1:8])
min_l1_norm = min(recorder[,9])
max_l1_norm = max(recorder[,9])
...
for(i in 1:8){
  ...
  points(l1_points[index_not_null-1], c(0), pch=21, bg=colors[i], cex=0.7)  
...
legend(2.3, 0.75, inset=c(-0.2,0), 
       legend=colnames(recorder[,1:8]), 
       cex = 0.7, col = colors, lty=1, title = "coefs")
